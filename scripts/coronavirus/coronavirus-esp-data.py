...
DataRepository.load_project_data_from_file(path="../../test_resource_files/coronavirus-esp.ods")

fitter = ModelFitter()
bc_fit = fitter.fit('barraza-contagion', 'corona1-esp', mts=False)
bc_fit.show_results()
...
data = DataRepository.provide_project_data('corona1-esp')
...
axes.plot(data.get_times(), data.get_cumulative_failures(), linewidth=2, color='blue',
                  label='Datos reales en España - 1/2/20 al 20/3/20')
