...
DataRepository.load_project_data_from_file(path="../../test_resource_files/coronavirus-arg.ods")

fitter = ModelFitter()
bc_fit = fitter.fit('barraza-contagion', 'corona1-arg', mts=False)
bc_fit.show_results()
...
data = DataRepository.provide_project_data('corona1-arg')
...
axes.plot(data.get_times(), data.get_cumulative_failures(), linewidth=2, color='blue',
                  label='Datos reales en Argentina - 4/3/20 al 17/3/20')
