...
DataRepository.load_project_data_from_file(path="../../../test_resource_files/covid_data_by_date/usa-27-03.ods")

fitter = ModelFitter()
bc_fit_40 = fitter.fit('barraza-contagion', 'corona1-usa-27-03', mts=False, end_sample=40)
bc_fit_45 = fitter.fit('barraza-contagion', 'corona1-usa-27-03', mts=False, end_sample=45)
bc_fit = fitter.fit('barraza-contagion', 'corona1-usa-27-03', mts=False)

fig, axes = plt.subplots()

data = DataRepository.provide_project_data('corona1-usa-27-03')
...
axes.plot(data.get_times(), data.get_cumulative_failures(), linewidth=2, color='blue',
          label='Datos reales en Estados Unidos - 21/1/20 al 27/3/20')
