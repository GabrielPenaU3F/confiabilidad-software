...
class LogisticPlotStrategy(PlotStrategy):
    ...
    def plot(self, project_name, lsq_params, ml_params):
        ...
        ax.plot(x_axis_data, log.calculate_mean_failure_numbers(x_axis_data, lsq_params[0], lsq_params[1]),
                linewidth=1, color='#ca3e47', linestyle='-', label='Least squares')
        ax.plot(x_axis_data, log.calculate_mean_failure_numbers(x_axis_data, ml_params[0], ml_params[1]),
