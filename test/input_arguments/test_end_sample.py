...
class TestEndSample(unittest.TestCase):
    ...
    def test_agile_n1_delayed_s_shaped_with_a_partial_dataset_of_25_samples(self):
        a = self.__class__.fit_ds_agile1_25samples.get_ml_parameters()[0]
        b = self.__class__.fit_ds_agile1_25samples.get_ml_parameters()[1]
        self.assertAlmostEqual(a, 70.209309, places=6)
        self.assertAlmostEqual(b, 0.003210, places=6)

    def test_agile_n1_logistic_with_a_partial_dataset_of_25_samples(self):
        a = self.__class__.fit_log_agile1_25samples.get_ml_parameters()[0]
        b = self.__class__.fit_log_agile1_25samples.get_ml_parameters()[1]
        c = self.__class__.fit_log_agile1_25samples.get_ml_parameters()[2]
        self.assertAlmostEqual(a, 28.432750, places=6)
        self.assertAlmostEqual(b, 0.013269, places=6)
        self.assertAlmostEqual(c, 256.198075, places=6)

    def test_mixed_delayed_s_shaped_with_a_partial_dataset_of_35_samples(self):
        a = self.__class__.fit_ds_mixed_35samples.get_ml_parameters()[0]
        b = self.__class__.fit_ds_mixed_35samples.get_ml_parameters()[1]
